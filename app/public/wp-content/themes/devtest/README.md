...
- Search for: `@package devtest` and replace with: `@package project_name` to capture the package name
- Update `"devtest\devtest,devtest"` to: `"CompanyName\project_name,project_name"` (with double quotes) in phpcs.xml.dist
- Search for: `devtest\devtest` and replace with: `CompanyName\project_name` to capture the namespaces
- Update `"devtest/devtest"` to `"companyname/project_name"` (with double quotes) in composer.json
- Search for: `, 'devtest'` and replace with: `, 'project_name'` (inside backticks) to capture the text domain
- Update `Text Domain: devtest` to: `Text Domain: project_name` in style.css
- Update `"devtest"` to: `"project_name"` (with double quotes) in phpcs.xml.dist and package.json
- Update `'devtest'` to: `'project_name'` (with single quotes) in inc/setup/setup.php
- Search for: `devtest_` and replace with: `project_name_` to capture all the function names
- Search for: `'devtest-` and replace with: `'project_name-` to capture prefixed handles
- Search for `devtest.pot` and replace with: `project_name.pot` to capture translation files
- Search for `devtest.local` and replace with: `project_name.test` to match your local development URL
